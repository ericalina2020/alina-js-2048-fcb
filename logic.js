...
// These variables will be used to monitor if the user wins
// If one of these variables value became true, the user wins
...
function setGame(){
	...
	for(let r = 0; r < rows; r++){
		for(let c = 0; c < columns; c++){
			...
			// Each title will have an id based on its row and column position through this code.
...
function slideLeft(){
	...
	for(let r = 0; r < rows; r++){
		...
		// We use the slide function to merge tiles with the same values
		row = slide(row);
...
function slideUp(){
	for(let c = 0; c < columns; c++){
		...
		// This loop updates the ids and colors of tiles
		for(let r = 0; r < rows; r++){
			board[r][c] = row[r];
...
function slideDown(){
	for(let c = 0; c < columns; c++){
		...
		// Checks which tiles have changed its position
...
function setTwo(){
    ...
    // If all tiles are not empty, it will not set a 2 value
    if(!hasEmptyTile()){
        return; // Because of the return keyword, it will not proceed to next codes inside the function. Therefore, it will not set two if all tiles is not empty.
    }
    ...
    while(!found){
    	...
    	// Gets a random tile based on random row and column
        let r = Math.floor(Math.random() * rows);
        let c = Math.floor(Math.random() * columns);


        // Checks the random tile in the board if its value is zero. If it is, then let's make it 2.
        if(board[r][c] == 0){
            board[r][c] = 2;
            let tile = document.getElementById(r.toString() + "-" + c.toString());
            tile.innerText = "2";
...
// This will listen when we touch the screen and assigns the x-coordinates of that touch
document.addEventListener('touchstart', (e) => {
	startX = e.touches[0].clientX;
	startY = e.touches[0].clientY;
});

// This will check where you touch your screen and prevents scrolling one you swwipe.
// Input targets any elements that include the word tile in their class name
